...
class ApiResponseHeader(BaseModel, Generic[T], ABC):
    ...
    messageCode: int = Field(
        ..., description=str(utils.MessageCodes.english_message_names)
    )
...
class APIErrorResponse(JSONResponse):
    ...
    def __init__(
        ...
        msg_code_params: dict[Any, Any] | list[Any] | None = None,
        **kwargs: dict[str, Any]
    ) -> None:
        header_data: dict[str, Any] = {
            "status": msg_status,
            "message": utils.MessageCodes.get_message(
                msg_code, msg_code_params=msg_code_params
            ),
            "messageCode": msg_code,
        }
        if header:
        ...
        super().__init__(self.response_data, **kwargs)  # type: ignore
